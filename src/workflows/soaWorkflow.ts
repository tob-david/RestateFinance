...
import { SoaProcessingType, SoaProcessingItem } from "../utils/types/soa";
import { IGetSoaJob, IGetSoaBatchStatus } from "../utils/types/customer";
...
import { downloadSoaFiles } from "../utils/storage";
...
export const soaWorkflow = restate.workflow({
  ...
  handlers: {
    run: async (ctx: WorkflowContext, type: documentTypes) => {
      ...
      const customers = await ctx.run("Step1-GetCustomers", async () => {
      ...
      // ========== STEP 2: Create Batch ==========
      const batchId = await ctx.run("Step2-CreateBatch", async () => {
      ...
      for (const customer of customerRows) {
        ...
        const processingItem: SoaProcessingItem = {
          customerId,
          timePeriod,
          processingDate,
          batchId,
          ...
          try {
            ...
            if (shouldDoReminder) {
                    ...
                    await ctx.run(
                      ...
                      async () => {
                        const reminderId = await createSoaReminder(
                          ...
                          processingItem.timePeriod,
                          branch.code,
                          branchResult.soaData as StatementOfAccountModel[]
                        );
                        console.log(
                ...
                if (singleResult.soaData && singleResult.soaData.length > 0) {
                  await ctx.run(`CreateReminder-${customerId}`, async () => {
                    const reminderId = await createSoaReminder(
                      ...
                      processingItem.timePeriod,
                      processingItem.branch,
                      singleResult.soaData as StatementOfAccountModel[]
                    );
                    console.log(
              ...
              while (currentReminderCount < maxReminders) {
                ...
                const reminderProcessingItem: SoaProcessingItem = {
                  ...processingItem,
      ...
      const finalStatus = await ctx.run("Step4-FinalizeBatch", async () => {
        const status = (await findBatchStatus(batchId)) as IGetSoaBatchStatus;
        ...
        if (totalDone >= status.TOTAL_CUSTOMERS) {
          await updateBatchStatus(batchId, result);
        }
